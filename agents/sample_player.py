...
class SamplePlayer(BasePokerPlayer):
    def __init__(self):
        self.hole = None
        self.community = None
        self.street = None
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if win_rate >= 0.75:
            if action_dict["raise"]["amount"]["min"] > 0:
                r = (win_rate - 0.75) / 0.25
                return "raise", int((action_dict["raise"]["amount"]["min"]) * (1 - r) + (action_dict["raise"]["amount"]["max"]) * r)
            return "call", action_dict["call"]["amount"]
        if my_stack - action_dict["call"]["amount"] + (self.max_round - round_state["round_count"]) * self.small_blind * 1.5 < self.initial_stack:
